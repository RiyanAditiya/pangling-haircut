<div>
    <flux:heading size="xl" level="1">{{ __('Tambah Kategori Layanan') }}</flux:heading>
    <flux:subheading size="lg" class="mb-6">
        {{ __('Tambah kategori layanan Pangling Haircut') }}
    </flux:subheading>
    <flux:separator variant="subtle" />

    <section class="bg-white dark:bg-gray-900 rounded-lg shadow p-6 mt-6 max-w-2xl">
    ...
    border-gray-300 bg-gray-50 dark:bg-gray-800 dark:border-gray-600"
                    x-data x-on:click="$refs.image.click()" x-on:dragover.prevent
                    x-on:drop.prevent="
        const file = $event.dataTransfer.files[0];
        if(file){
            $refs.image.files = $event.dataTransfer.files;
            $refs.image.dispatchEvent(new Event('change'));
        }
    ">
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-300">
                        Tarik & letakkan gambar atau <span class="text-indigo-600 font-medium">klik untuk upload</span>
                    </p>
                </div>
