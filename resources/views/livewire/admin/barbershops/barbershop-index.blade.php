<div>
    <div class="relative mb-6 w-full">
        ...
        <flux:heading size="xl" level="1">{{ __('Data Barbershops') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Kelola data barbershops Pangling Haircut') }}
        </flux:subheading>
                        ...
                        <form wire:submit="search" class="flex items-center">
                            ...
                            <div class="relative w-full">
                                ...
                                <input wire:model.live.debounce.250ms="query" type="text" id="simple-search"
                                    ...
                                    placeholder="Cari barbershop berdasarkan nama barbershop...">
                            </div>
                        </form>
                    </div>
                    <div
                        class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                        <a wire:navigate href="{{ route('admin.barbershopCreate') }}" id="createProductModalButton"
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                <th scope="col" class="px-4 py-4">Nama Barbershop</th>
                        ...
                        <tbody>
                            @forelse ($barbershops as $barbershop)
                                <tr class="border-b dark:border-gray-700">
                                    <td class="px-4 py-3">{{ $barbershop->name }}</td>
                                    <td class="px-4 py-3">{{ $barbershop->address }}</td>
                                    <td class="px-4 py-3">{{ $barbershop->phone }}</td>
                                    <td class="px-4 py-3 flex items-center justify-end">
                                        <a wire:navigate href="{{ route('admin.barbershopEdit', $barbershop->id) }}"
                                            class="focus:outline-none text-white bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-xs px-3 py-2 me-1 mb-2 dark:focus:ring-yellow-900">Edit</a>
                                        <button wire:click="confirmDelete({{ $barbershop->id }})"
        ...
        @if ($deleteId)
            <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
                <div class="bg-white rounded-lg shadow-lg w-96 p-6">
                    ...
                    <p class="mt-2 text-gray-600">Apakah Anda yakin ingin menghapus barbershop ini?</p>
