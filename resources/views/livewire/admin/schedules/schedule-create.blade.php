<div>
    <flux:heading size="xl" level="1">{{ __('Tambah Jadwal') }}</flux:heading>
    <flux:subheading size="lg" class="mb-6">
        {{ __('Tambah jadwal Pangling Haircut') }}
    </flux:subheading>
    <flux:separator variant="subtle" />

    <section class="bg-white dark:bg-gray-900 rounded-lg shadow p-6 mt-6">
        <form wire:submit.prevent="save" class="space-y-5" x-data="{ isDayOff: @entangle('is_day_off') }">
            ...
            <div x-show="!isDayOff" x-transition class="space-y-5">
                ...
                <!-- Slot Duration -->
                <div>
                    <label for="slot_duration" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                        Durasi Slot (menit)
                    </label>
                    <input type="number" id="slot_duration" wire:model="slot_duration" min="15" max="180"
                        step="15"
                        class="w-full rounded-lg border border-gray-300 p-2.5 text-sm text-gray-900
                               dark:bg-gray-700 dark:border-gray-600 dark:text-white
                               focus:ring-primary-500 focus:border-primary-500">
